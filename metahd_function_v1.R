...
MetaHD <- function(Y,Slist,Psi = NULL,shrinkCor = TRUE,method = c("reml","fixed"),bscov = c("unstructured","diag"),imputation = c("none","low.weight","mean","eimpute","knn"),rigls.maxiter = 5){
  N <- ncol(Y)
  K <- nrow(Y)
  q <- p <- 1
  nall <- N*K
  ...
  B <- matrix(0, nrow = N)
  Imp <- match.arg(imputation)
  method <- match.arg(method)
  bscov <- match.arg(bscov)
  S <- t(sapply(Slist,vechMatrix))
  if (Imp == "none"){
    y <- Y
  }else if(Imp == "low.weight"){
    data <- low.weight(Y,Slist)
    y <- data$effects
    Slist <- data$wscovar
    for (k in 1:K) {
    ...
    Y_impute <- eimpute(Y,r=2)
    y <- Y_impute$x.imp
  }else if(Imp == "knn"){
    Y_impute <- impute.knn(t(Y),colmax = 1)
    y <- t(Y_impute$data)
  }
  ...
  Xlist <- lapply(1:K, function(id) {
    diag(1, nrow = N, ncol = N)
  })
  ylist <- unlist(apply(y, MARGIN = 1, FUN = list), recursive = FALSE)
  nay <- is.na(y)
  ...
  if(is.null(Psi)){
    ...
    nalist <- lapply(1:nrow(y), function(j) {
      is.na(y[j,])
    })
