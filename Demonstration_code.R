...
# Read the data
Effects <- read_excel("simulated_data.xlsx", sheet = "effect_sizes")
WSVar <- read_excel("simulated_data.xlsx", sheet = "within-study_variances")
WSCorr <- read_excel("simulated_data.xlsx", sheet = "within-study_correlations") # UPPER TRIANGULAR ELEMENTS
K <- nrow(Effects)
...
model <- MetaHD(Y,
                ...
                method = "reml",
                bscov = "unstructured")
model$estimate # TRUE VALUES ARE: 5, 1, 3, 2, 4, 2, 1, 1, 5, 1, 1, 5, 5, 2, 3, 5, 4, 5, 1, 1, 2, 5, 1, 2, 1, 5, 4, 1, 4, 2
